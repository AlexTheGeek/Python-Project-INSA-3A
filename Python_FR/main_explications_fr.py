...
###
#Récupération des données séparées par des virgules à partir d'un fichier csv pour générer un échantillon de manière aléatoire, et ainsi afficher des informations sur cet DataFrame (dimension, statistique, utilisation mémoire, valeurs non nulles, colonnes, index dtype, ...)
###
...
###
#Affichage de la répartition des températures en fonction des températures minimales et températures maximales, dans un graphique en nuage de points
###
dataset.plot(x='Temp_Min', y='Temp_Max', style='o') #Création d'un tracé de DataFrame, avec l'axe x représentant les Temp_min et l'axe y représentant les Temp_Max, dans un style de nuage de points
...
###
#Création d'un graphique histogramme pour visualiser la répartion des valeurs de Temp_Max du DataFrame
###
...
seabornInstance.distplot(dataset['Temp_Max'])   #Permet de dessiner un tracé de distribution sur une FacetGrid, permettant de visualiser les données de DataFrame des Temp_Max dans un format d'histogramme (par défaut)
plt.show()                                      #Affichage du graphique/figure
...
reg = LinearRegression().fit(X_train, y_train)                                                      #Entrainement du modéle en utilisant l'ensemble de formation
...
df = pd.DataFrame({'Actuelle(Mesurées)': y_test.flatten(), 'Prédiction(modèle)': y_pred.flatten()}) #Construction d'un DataFrame à partir des données prédites et de tests
...
plt.xlabel('Temp_Min')                              #Paramétrage du titre de l'axe x
plt.ylabel('Temp_Max')                              #Paramétrage du titre de l'axe y
plt.scatter(X_test, y_test,  color='gray')          #Création d'un diagramme de dispersion y_test par rapport à X_test de couleur grise
plt.plot(X_test, y_pred, color='red', linewidth=2)  #Création d'un tracé (de la fonction ax+y) de DataFrame, avec l'axe x représentant les X_Test et l'axe y représentant les y_pred, dans la couleur rouge et de largeur 2points
plt.show()                                          #Affichage du graphique/figure
...
###
#Ajout des valeurs moyennes dans un tableau, affichage de ce tableau et des valeurs moyennes et écart-type
###
Means = []                                          #Création d'un tableau vide des valeurs moyennes
Means=testSamples(200, 100,dataset['Temp_Min'])     #Ajout des valeurs moyennes dans le tableau avec la fonction testSamples créée précédemment
...
###
stat, p = stats.shapiro(Means)                          #On fait le test de Shapiro-Wilk qui vérifie l’hypothèse nulle selon les données de Means, et retourne la valeur de la statistique du test et la p-value pour l'hypothèse du test
...
###
#Diagramme en boites et à moustaches. La boîte s'étend des valeurs du quartile inférieur au quartile supérieur des données, avec une ligne à la médiane. Les moustaches s'étendent à partir de la boîte pour montrer l'étendue des données. Les points de vol sont ceux qui se trouvent après l'extrémité des moustaches.
###
...
print('LogisticRegression Mean Accuracy: %.3f (%.3f)' % (np.mean(scores1), np.std(scores1)))              #Affichage du coefficient de régression linéaire moyen pour le modèle 1   
...
print('LinearDiscriminantAnalysis Mean Accuracy: %.3f (%.3f)' % (np.mean(scores2), np.std(scores2)))      #Affichage du coefficient de régression linéaire moyen pour le modèle 2 
...
X_lda = lda.transform(X)                                                #Transforme les valeurs pour qu'elles soient utilisables par les fonctions d'après
plt.figure(figsize=(6, 4))                                              #Création d'une figure avec une certaine taille précisée en argument
for lab, col in zip((0, 1),('blue', 'red')):                            #Boucle pour tracer le nuage de points en fonction de la ligne et de la colonne
    plt.scatter(X_lda[y == lab, 0],X_lda[y == lab, 1],label=lab,c=col)  #Traçage du nuage de points
